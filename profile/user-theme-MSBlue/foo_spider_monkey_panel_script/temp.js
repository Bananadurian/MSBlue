...
const TEXT_FLAGS = DT_CENTER | DT_VCENTER | DT_NOPREFIX | DT_END_ELLIPSIS | DT_SINGLELINE;
...
const STAR_ICONS_DIR = fb.ProfilePath + "\\user-theme-MSBlue\\imgs\\Lucide\\";
const STAR_ICONS = {
...
class StarElement {
    ...
    paint(gr) {
        ...
        if (icon) {
            gr.DrawImage(icon, this.x, this.y, this.w, this.h, 0, 0, icon.Width, icon.Height);
        }
    }
...
// 文本内容状态对象
const CONTENTS = {
    title:  { text: "", w: 0, x: 0, y: 0, h: LINE_H, is_hover: false, tooltip: ""},
    artist: { text: "", w: 0, x: 0, y: 0, h: LINE_H, is_hover: false, tooltip: ""},
    album:  { text: "", w: 0, x: 0, y: 0, h: LINE_H, is_hover: false, tooltip: ""},
...
function measure_string(text, font, maxWidth, text_style_flag) {
    ...
    const result = _measureGr.MeasureString(text, font, 0, 0, maxWidth, MAX_BUFFER_H, text_style_flag);
...
function update_contents() {
    ...
    if (g_metadb) {
        CONTENTS.title.text = tf_track_title.EvalWithMetadb(g_metadb) || "Unknown Title";
        CONTENTS.artist.text = tf_track_artist.EvalWithMetadb(g_metadb) || "Unknown Artist";
        CONTENTS.album.text = tf_track_album.EvalWithMetadb(g_metadb) || "Unknown Album";
...
function update_source_icon(metadb) {
    const sourceText = tf_album_source.EvalWithMetadb(metadb).trim().toUpperCase();
...
function on_size() {
    ...
    // 2. 垂直布局计算 (自上而下)
    const totalContentH = CONTENTS.title.h + (LINE_H * 2) + (MARGIN * 4) + STAR_SIZE + SOURCE_ICON_SIZE;
    ...
    // --- Album & Year ---
    let albumYearTotalW = CONTENTS.album.w + (CONTENTS.year.text ? (ALBUM_YEAR_GAP + CONTENTS.year.w) : 0);
    if (albumYearTotalW > max_text_w) {
        albumYearTotalW = max_text_w;
        CONTENTS.album.w = max_text_w - (CONTENTS.year.text ? (ALBUM_YEAR_GAP + CONTENTS.year.w) : 0);
    }
    ...
    if(currentAQBadge) {
        const badgeTextSize = measure_string(currentAQBadge.label, FONTS.BadgeLabel, max_text_w, BADGE_TEXT_ALIGN);
    ...
    currentAQBadgeRect.y = startY + Math.ceil(((LINE_H - currentAQBadgeRect.h) / 2));
}
...
function on_paint(gr) {
    ...
    // --- 绘制文本 ---
    gr.GdiDrawText(CONTENTS.title.text, FONTS.Title, COLORS.Title, CONTENTS.title.x, CONTENTS.title.y, CONTENTS.title.w, CONTENTS.title.h, TEXT_FLAGS);
    ...
    if (CONTENTS.year.text) {
        gr.GdiDrawText(CONTENTS.year.text, FONTS.Body, COLORS.Body, CONTENTS.year.x, CONTENTS.year.y, CONTENTS.year.w, CONTENTS.year.h, ALBUM_FLAGS);
    }
    
    // --- 绘制星星 ---
    // 先清理背景防止残影
    gr.FillSolidRect(ratingArea.x, ratingArea.y, ratingArea.w, ratingArea.h, COLORS.bg);
    ...
    if (g_activeElement) {
        if (g_activeElement instanceof StarElement) {
            ...
            if (g_activeElement === CONTENTS.artist) {
                // 艺人行可能比较宽，重绘整行更安全
                window.RepaintRect(0, g_activeElement.y, window.Width, g_activeElement.h);
            } else {
                window.RepaintRect(g_activeElement.x, g_activeElement.y, g_activeElement.w, g_activeElement.h);
            }
        }
...
function on_mouse_lbtn_up(x, y) {
    ...
    if (target instanceof StarElement) {
        ...
        const newRating = (target.value === g_currentRating) ? "<not set>" : target.value;
        fb.RunContextCommandWithMetadb("Playback Statistics/Rating/" + newRating, g_metadb, 8);
        return;
    }
...
// 播放与列表事件回调
function on_playback_new_track() { update_contents(); }
function on_playback_stop(reason) { if (reason !== 2) update_contents(); }
function on_item_focus_change() { if (!fb.IsPlaying) update_contents(); }
function on_playlist_switch() { if (!fb.IsPlaying) update_contents(); }
function on_metadb_changed() { update_contents(); }
